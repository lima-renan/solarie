...
const CartItemCard = ({ product, isSearch, setSearch }) => {
  ...
  const { updateProductQtyInCart, deleteProductFromCart, disableCart } =
    useCartContext();
  const { addProductToWishlist, deleteProductFromWishlist, disableWish, isInWish } =
    useWishlistContext();

  const updateHandler = (type) => {
    if (type === "increment" && product.quantity > product.qty) {
        updateProductQtyInCart(product.product ? product.product : product._id, type);
    } else if (product.qty > 1) {
        updateProductQtyInCart(product.product ? product.product : product._id, type);
    } else {
        deleteProductFromCart(product.product ? product.product : product._id);
    }
  };
  let inWish;
  if(product.product){
    inWish = isInWish(product.product);
  }
  else{
    inWish = isInWish(product._id);
  }
  ...
  return (
    <div
      ...
      onClick={() => {
        if (isSearch) {
          setSearch("");
          navigate(`product/${product.product ? product.product : product._id}`);
        }
      }}
    >
      <div className="flex  items-center flex-wrap gap-2 w-full">
        <div className="flex flex-wrap xs:flex-nowrap justify-center xs:justify-start flex-1 items-center gap-5">
          ...
          >
            <img src={product.imageUrl} alt="" className="object-fit w-full" />
          </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-1 sm:gap-3  ">
                  <button
                    ...
                    disabled={disableCart}
                    onClick={() => deleteProductFromCart(product.product ? product.product : product._id)}
                  >
                    Remover da Bag
                  </button>
                  <button
